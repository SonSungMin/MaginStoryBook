document.addEventListener('DOMContentLoaded', () => {
    ...
    if (currentUser.role === 'teacher' || currentUser.role === 'director' || currentUser.role === 'admin') {
        teacherToolsNavButton.style.display = 'flex'; // 선생님 도구함 메뉴 보이기
        teacherToolsSection.style.display = 'block'; // 선생님 도구함 섹션 보이기
    } else {
        teacherToolsNavButton.style.display = 'none'; // 그 외 권한은 숨김
        teacherToolsSection.style.display = 'none';
    }


    // --- 데이터 관리 (localStorage 기반) ---
    // myStories 데이터는 로그인한 사용자의 ID와 연결하여 관리
    let myStories = []; 

    // 앱 로드 시 localStorage에서 데이터 불러오기
    function loadStories() {
        const storedStories = localStorage.getItem('magicStorybookStories');
        if (storedStories) {
            myStories = JSON.parse(storedStories);
        } else {
            // 초기 샘플 데이터 (AI 변환 완료된 상태 가정)
            // 실제 구현 시 각 사용자에 맞는 데이터만 로드
            myStories = [
                {
                    id: 'story-1',
                    uploaderId: 'teacher1', // 샘플 데이터의 업로더
                    title: '다정 선생님',
                    originalImg: 'images/original_drawing_teacher.png',
                    aiImg: 'images/ai_drawing_teacher_v1.png',
                ...
                {
                    id: 'story-2',
                    uploaderId: 'student1', // 샘플 데이터의 업로더
                    title: '우리집 강아지',
                    originalImg: 'images/original_drawing_dog.png',
                    aiImg: 'images/ai_drawing_dog_v1.png',
                 ...
                 {
                    id: 'story-3',
                    uploaderId: 'student2', // 샘플 데이터의 업로더
                    title: '놀이터에서',
                    originalImg: 'images/original_drawing_kids_play.png',
                    aiImg: 'images/ai_drawing_kids_play_v1.png',
                    storyText: '친구들이랑 놀이터에서 뛰어 놀았어요. 미끄럼틀이 제일 재밌어요!',
                    date: '2024.05.01',
                    aiProcessed: true
                }
            ];
            saveStories(); // 초기 샘플 저장
        }
    }

    // 데이터 localStorage에 저장
    function saveStories() {
        localStorage.setItem('magicStorybookStories', JSON.stringify(myStories));
    }
    ...
    function activateSection(targetId) {
        ...
        if (targetSection) {
            ...
            if (targetId === 'my-story') {
                renderMyStoryCards(); // 내 그림 이야기 갱신
            } else if (targetId === 'class-story') {
                renderClassStoryCards(); // 우리 반 이야기 갱신
            } else if (targetId === 'teacher-tools') {
                if (currentUser.role === 'teacher' || currentUser.role === 'director' || currentUser.role === 'admin') {
                    renderTeacherArtworkList(); // 선생님 도구함 목록 갱신 (권한 있는 경우만)
                } else {
                    alert('선생님 도구함에 접근할 권한이 없습니다.');
                    // 권한 없는 경우 기본 섹션으로 리다이렉션 (예: 내 그림 이야기)
    ...
    window.handleLogout = function() {
        sessionStorage.removeItem('loggedInUser'); // 세션에서 사용자 정보 제거
        alert('로그아웃되었습니다.');
    ...
    function renderMyStoryCards() {
        ...
        // 현재 로그인한 사용자의 작품만 필터링하여 표시
        const userStories = myStories.filter(story => story.uploaderId === currentUser.name);
    ...
    function createStoryCardElement(story, includeInteraction = false) {
        ...
        img.src = story.aiImg; // AI 변환된 이미지를 메인으로
        img.alt = story.title;
    ...
    function renderTeacherArtworkList() {
        teacherArtworkList.innerHTML = ''; // 목록 초기화
        ...
        myStories.forEach(story => {
            ...
            artworkItem.dataset.originalImg = story.originalImg;
            artworkItem.dataset.aiImg = story.aiImg;
            artworkItem.dataset.title = story.title;
            artworkItem.dataset.storyText = story.storyText;

            artworkItem.innerHTML = `
                <img src="${story.aiImg}" alt="${story.title}">
                <span>${story.title} (${story.uploaderId})</span>
                ${story.aiProcessed ? '<i class="fas fa-magic ai-icon" title="AI 변환 완료"></i>' : '<i class="fas fa-spinner fa-spin ai-icon" title="AI 변환 중"></i>'}
            `;
            teacherArtworkList.appendChild(artworkItem);
        });
        attachDragAndDropListeners(); // 새로 렌더링된 요소에 드래그 앤 드롭 리스너 다시 붙이기
    }
    ...
    window.previewOriginalDrawing = function(event) {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                ...
                currentOriginalDrawingSrc = e.target.result; // 원본 이미지 경로 (Base64) 저장
            };
    ...
    window.processDrawingWithAI = function() {
        ...
        if (!currentOriginalDrawingSrc || currentDrawingTitle === '') {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    // 시뮬레이션: 샘플 AI 변환 이미지를 보여줌
                    // 실제 구현 시 백엔드 AI API 호출 후 결과 이미지 URL을 받아옴
                    let simulatedAiImg = 'images/sample_ai_generated_default.png'; // 기본 fallback
                    if (currentOriginalDrawingSrc.includes('teacher')) { // 경로에 'teacher'가 포함된 경우
                         simulatedAiImg = 'images/ai_drawing_teacher_v1.png';
                    } else if (currentOriginalDrawingSrc.includes('dog')) { // 경로에 'dog'가 포함된 경우
                        simulatedAiImg = 'images/ai_drawing_dog_v1.png';
                    } else if (currentOriginalDrawingSrc.includes('kids_play')) {
                        simulatedAiImg = 'images/ai_drawing_kids_play_v1.png';
                    } else {
                        // 새로운 업로드 이미지에 대한 AI 변환 시뮬레이션 (랜덤)
                        const samples = ['images/ai_drawing_teacher_v1.png', 'images/ai_drawing_dog_v1.png', 'images/ai_drawing_kids_play_v1.png', 'images/sample_ai_generated_default.png'];
                        simulatedAiImg = samples[Math.floor(Math.random() * samples.length)];
                    }
    ...
    window.closeAIResultModal = function() {
        ...
        compareToggle.checked = false; // 항상 초기화
    };

    window.selectAIImage = function() {
        // AI가 생성한 이미지를 최종 이미지로 선택하고 데이터에 추가
        const newStory = {
            id: 'story-' + Date.now(), // 고유 ID 부여
            uploaderId: currentUser.name, // 현재 로그인 사용자 ID 저장
            title: currentDrawingTitle,
            originalImg: currentOriginalDrawingSrc, // 원본 Base64 또는 업로드된 파일 URL
            aiImg: aiGeneratedImg.src, // AI 생성 이미지 URL
            storyText: currentDrawingStory,
            date: new Date().toLocaleDateString('ko-KR'),
            aiProcessed: true
        };
        myStories.unshift(newStory); // 최신 작품을 배열 맨 앞에 추가
        saveStories();

        alert('마법 그림이 내 그림 이야기에 추가되었습니다!');
        closeAIResultModal();
        renderMyStoryCards(); // '내 그림 이야기' 섹션 갱신
        if (currentUser.role === 'teacher' || currentUser.role === 'director' || currentUser.role === 'admin') {
            renderTeacherArtworkList(); // 선생님 도구함 갱신 (권한 있는 경우만)
        }
    };
    ...
    compareToggle.addEventListener('change', () => {
        // 이 시뮬레이션에서는 토글과 관계없이 AI 변환된 이미지 항상 표시
        // 실제 구현 시 toggle.checked 상태에 따라 originalImg와 aiGeneratedImg의 opacity/display를 조작하여 비교 효과 구현
    });
    ...
    function openStoryDetailModal(storyId) {
        ...
        detailOriginalImg.src = story.originalImg;
        detailAiImg.src = story.aiImg;
        detailStoryText.textContent = story.storyText;
        detailStoryDate.textContent = story.date;
    ...
    // --- 드래그 앤 드롭 기능 (선생님 도구함) ---
    let draggedItem = null; // 드래그되는 아이템 저장
    ...
    window.exportStorybook = function() {
        alert('PDF 내보내기 기능은 백엔드에서 PDF 생성 로직이 필요합니다! 구성된 페이지를 서버로 전송해야 합니다.');
    };
